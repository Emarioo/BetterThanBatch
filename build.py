...
def build_btb(config: BuildConfig) -> bool:
    ...
    for srcdst in files:
        ...
        obj_file = srcdst[1]
        if config.toolchain == 'msvc':
            FLAGS = f"/std:c++17 /nologo /EHsc /TP /wd4129 /Isrc /Iinclude /I{TRACY_DIR}/public /FI Engone/pch.h /DCOMPILER_MSVC"
            if platform.system() == "Windows":
                FLAGS += " -DOS_WINDOWS"
            elif platform.system() == "Linux":
                FLAGS += " -DOS_LINUX"
            ...
            FLAGS = f"-std=c++17 -Isrc -Iinclude -I{TRACY_DIR}/public -include src/Engone/pch.h -DCOMPILER_GNU"
...
def release_btb(config: BuildConfig, version):
    ...
    commit_file = "src/basin/const_commit.cpp"
    ...
    if current_text != wanted_text:
        with open("src/basin/const_commit.cpp", "w") as f:
            f.write(wanted_text)
...
def gather_btb_files(config: BuildConfig) -> list[tuple[str,str]]:
    ...
    for file in glob.glob("src/**/*.cpp", recursive = True):
        ...
        if file.find("/basin/") == -1 and file.find("/Engone/") == -1:
            continue
...
def find_includes(file):
    ...
    if not os.path.exists(file):
        ...
        if file.find("basin") != -1 or file.find("Engone") != -1:
            # If we don't find files here then we have a problem
