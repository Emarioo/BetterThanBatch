#include "basin/machine/_deprecated/PDB.h"
...
bool PDBFile::writeFile(const char* path) {
    ...
    */
    ...
    u32 streamDirectory_block = requestBlock();
    u32 second_streamTable_block = requestBlock();
    Assert(streamDirectory_block != -1);
    Assert(second_streamTable_block != -1);
    MSFHeader* msfHeader = nullptr;
    {
        msfHeader = (MSFHeader*)(_baseData);
        new(msfHeader)MSFHeader();
        // msfHeader->blockCount = ; // not fully known yet
        msfHeader->blockOfActiveFPM = 2;
        memset(_baseData + 2 * BLOCK_SIZE, 0xFF, BLOCK_SIZE); // clear FPM 2
        // if(msfHeader->blockOfActiveFPM == 1) {
        //     // memset(_baseData + 1 * BLOCK_SIZE, 0, BLOCK_SIZE); // clear FPM 1
        //     // memset(_baseData + 2 * BLOCK_SIZE, 0xFF, BLOCK_SIZE); // clear FPM 2
        //     setFreeBlock(0, false, 2);
        //     setFreeBlock(1, false, 2);
        //     setFreeBlock(2, false, 2);
        // } else {
        //     memset(_baseData + 1 * BLOCK_SIZE, 0xFF, BLOCK_SIZE); // clear FPM 1
        //     // memset(_baseData + 2 * BLOCK_SIZE, 0xFF, BLOCK_SIZE); // clear FPM 2
        //     setFreeBlock(0, false, 1);
        //     setFreeBlock(1, false, 1);
        //     setFreeBlock(2, false, 1);
        // }
        ...
        WRITE(u32, streamDirectory_block);
    }
    // streams[1].size+=4;
    {
        u8* ptr = _baseData;
        u32 offset = streamDirectory_block * BLOCK_SIZE;
        ...
        streams[0].blockIndices[0] = second_streamTable_block;
        FOR(streams) {
    ...
    {
        u8* ptr = _baseData;
        u32 offset = second_streamTable_block * BLOCK_SIZE;
        u32 baseOffset = offset;
        ...
        streams[0].blockIndices.resize(1);
        streams[0].blockIndices[0] = streamDirectory_block;
        FOR(streams) {
            for (int i=0;i<it.blockIndices.size();i++) {
                WRITE(u32, it.blockIndices[i]);
            }
...
u32 PDBFile::requestBlock() {
    ...
    // u8* fpm = _baseData + activeFPM * BLOCK_SIZE;
...
bool PDBFile::reserveTotalBlocks(u32 newBlockCount) {
    ...
    if(!_baseData) {
        ...
        activeFPM = 1;
        u32 otherFPM = 2;
        memset(_baseData + activeFPM * BLOCK_SIZE, 0xFF, BLOCK_SIZE);
...
bool PDBFile::setFreeBlock(i32 blockIndex, bool beFree, int fpmToUse) {
    if(fpmToUse == -1) // default
        fpmToUse = activeFPM;
    if(_blockCount <= fpmToUse)
        return false;
    
    u32 byte = blockIndex >> 3;
    u32 bit = blockIndex & 7;
    u8* fpm = _baseData + fpmToUse * BLOCK_SIZE;
...
PDBFile* PDBFile::Deconstruct(const char* path) {
    ...
    log::out << "Block number of valid FPM "<<msfHeader->blockOfActiveFPM <<" (altough we switch to 1)\n";
    pdb->activeFPM = msfHeader->blockOfActiveFPM;
    ...
    Assert(msfHeader->bytesOfStreamTable / msfHeader->blockSize == 0);
    ...
    memset(pdb->_baseData + 2*BLOCK_SIZE, 0xFF, BLOCK_SIZE);
    memset(pdb->_baseData + BLOCK_SIZE, 0, BLOCK_SIZE);
...
//     log::out << "Block number of valid FPM "<<pdb->msfHeader->blockOfActiveFPM <<"\n";
