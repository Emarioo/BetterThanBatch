...
void Context::execute(Bytecode& code, Performance* perf){
    ...
    while(true){
            ...
            while (true) {
                ...
                if (t->state == UserThread::Active){
                    if(currentThread!=newIndex){
                        _ILOG_THREAD(log::out<<"Switching thread "<<currentThread<<" -> "<<newIndex<<"\n";)
                ...
                if (newIndex == currentThread) {
                    if(waitForOSThread){
                        _ILOG_THREAD(log::out << "Waiting for OSThread to finish\n";)
                        fullContinue=true;
        ...
        if(activeCode.length() == programCounter){
            ...
            _ILOG_THREAD(log::out<<"Thread "<<currentThread<<" finished\n";)
            exitScope();
            continue;
        }
        ...
        switch(inst.type){
            case 0:{
                _ILOG(log::out << LINST<<", skipping null instruction\n");
                continue;
            }
            case BC_AND:
            case BC_OR: {
                ...
                if(out){
                    _ILOG(log::out << LINST <<",  "<<left<<"?"<<right<<" yes\n";)
                }else{
                    _ILOG(log::out << LINST <<", "<<left<<"?"<<right<<"  no\n";)
                }
            ...
            case BC_NOT_EQUAL:{
                ...
                if(r0.type==REF_NUMBER && r1.type == REF_NUMBER && r2.type == REF_NUMBER){
                    ...
                    _ILOG(log::out << " ";PrintRefValue(this,r1);log::out<<" = "<<num<<"\n";)
                    v2->value = num;
                    ...
                    if(inst.type==BC_ADD){
                        ...
                        v2->memory.used = size;
                        _ILOG(log::out << LINST <<", \"";PrintRefValue(this,r2);log::out<<"\"\n";)
                    } else if(inst.type==BC_EQUAL){
                        if(v0->memory.used!=v1->memory.used){
                            v2->memory.used=0;
                            _ILOG(log::out << LINST <<",  no\n";)
                            continue;
                        }
            ...
            case BC_NOT:{
                ...
                if(r0.type==REF_NUMBER && r1.type == REF_NUMBER){
                    ...
                    v1->value = v0->value == 0;
                    _ILOG(log::out << LINST <<", not '";PrintRefValue(this,r1);log::out<<"'\n";)
            ...
            case BC_SUBSTR: {
                ...
                if(r0.type==REF_STRING && r1.type == REF_NUMBER && r2.type == REF_NUMBER){
                    ...
                    _ILOG(log::out <<log::AQUA << LINST <<", substr '"<<*out<<"'\n";)
            ...
            case BC_SETCHAR: {
                ...
                if(r0.type==REF_STRING && r1.type == REF_NUMBER && r2.type == REF_STRING){
                    ...
                    *((char*)v2->memory.data + index) = chr;
                    ...
                    _ILOG(log::out << LINST <<", set '"<< chr <<" at "<<index<<"'\n";)
                    // now the first register contains the substring of the original
                }else{
                    CERR << "invalid types "<<
                        RefToString(r0.type)<<" "<<RefToString(r1.type)<<" "<<RefToString(r2.type)<<" in registers\n";   
            ...
            case BC_LEN: {
                ...
                if(r0.type==REF_STRING && r1.type==REF_NUMBER){
                    ...
                    _ILOG(log::out << LINST <<", len "<<v1->value<<"\n";)
                }else{
                ...
                switch(r0.type){
                    case REF_STRING:
                    {
                        ...
                        if(!v0||!v1){
                            ...
                            v0->copy(v1);
                            _ILOG(log::out << log::AQUA<< LINST <<" ["<<r1.index<<"], copied ";PrintRefValue(this,r1);log::out<<"\n";)  
                        }
                        break;
                    }
                    case REF_NUMBER:
                    {
                        ...
                        if(!n0||!n1){
                            ...
                            n1->value = n0->value;
                            _ILOG(log::out <<log::AQUA << LINST <<" ["<<r1.index<<"], copied "<< n1->value <<"\n";)
                        }
                        break;
                    }
                    default:
                        r1 = r0;
                        _ILOG(log::out << LINST <<" ["<<r1.index<<"], copied null\n";)  
            ...
            case BC_MOV:{
                ...
                _ILOG(log::out << LINST <<", moved ";)
                if(r0.type==REF_NUMBER){
                    Number* v0 = getNumber(r0.index);
                    // if(v0)
                    //     _ILOG(log::out << v0->value)
                    // else
                    //     _ILOG(log::out << "?")
                    _ILOG(PrintRefValue(this,r0);)
            ...
            case BC_NUM: {
                ...
                _ILOG(log::out << log::AQUA<< LINST <<" ["<<r0.index<<"]\n";)
            ...
            case BC_DEL: {
                ...
                _ILOG(log::out << log::PURPLE << LINST <<" ["<<r0.index<<"]\n";)
                r0 = {};
                break;
            }
            case BC_LOADNC: {
                ...
                if (r0.type == REF_NUMBER){
                    ...
                    n0->value = num->value;
                    _ILOG(log::out << LINST << ", reg = " << n0->value<<"\n";)
                }else{
                    CERR << "invalid type "<<RefToString(r0.type)<<" in registers\n";   
                }
                break;
            ...
            case BC_LOADSC:{
                ...
                if (r0.type == REF_STRING){
                    ...
                    vc->copy(v0);
                    _ILOG(log::out << LINST << ", reg = ";PrintRawString(*v0);log::out<<"\n";)
                }else{
                    CERR << "invalid type "<<RefToString(r0.type)<<" in registers\n";   
                }
                break;
            ...
            case BC_LOADV:{
                ...
                r0 = *ref;
                _ILOG(log::out << LINST << ", loaded '";PrintRefValue(this,r0);log::out<<"' from "<<index<<"\n";)
                break;
            }
            case BC_STOREV:{
                Ref& r0 = references[inst.reg0];
                ...
                *ref = r0;
                ...
                _ILOG( if(deletedValue) log::out << log::PURPLE;
            ...
            case BC_JUMP: {
                ...
                if(r0.type==REF_NUMBER){
                    ...
                    if(n0->value != (Decimal)(uint)n0->value){
                        ...
                        _ILOG(log::out << "ContextWarning at "<<programCounter<<", " << inst << ", jumping to next instruction is redundant\n";)
                    }else{
                        _ILOG(log::out << LINST << ", jumped to "<<address<<"\n";)
                ...
                if(r1.type==REF_NUMBER){
                    ...
                    if(r0.type==REF_NUMBER||r0.type==REF_STRING){
                        ...
                        if(willJump){
                            ...
                            _ILOG(log::out << LINST << ", no jumping\n";)
                        }
                    } else{
            ...
            case BC_THREAD: {
                ...
                if (r1.type==REF_NUMBER){
                    ...
                    if(func){
                        ...
                        _ILOG(log::out << log::AQUA << LINST << ", external: '"<<name<<"' arg: '";
                        ...
                        _ILOG(log::out << log::AQUA << LINST << ", exe: '"<<name<<"' arg: '";
                        PrintRefValue(this,r0); log::out<<"'\n";)
            ...
            case BC_JOIN: {
                ...
                _ILOG(log::out << LINST << ", wait for thread "<<v0->value<<"\n";)
            ...
            case BC_RETURN: {
                ...
                if(currentScope<0){
                    ...
                    _ILOG(log::out << LINST << ", jumped to "<<programCounter<<", returned ";
            ...
            case BC_READ_FILE: {
                ...
                _ILOG(log::out << LINST << ", from: '"<<*v1<<"' content: '";PrintRefValue(this,r0);log::out <<"'\n";)
                break;
            }
            case BC_RUN: {
                Ref& r0 = references[inst.reg0];
