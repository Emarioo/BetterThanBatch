...
X64Register X64Builder::alloc_register(int artifical, X64Register reg,
                                       bool is_float) {
  using namespace engone;
// #define DO_LOG(X) X
#define DO_LOG(X)
...
void X64Builder::emit_modrm_sib(u8 mod, X64Register _reg, u8 scale, u8 index,
                                X64Register _base) {
  u8 base = _base - 1;
  u8 reg = _reg - 1;
...
void X64Builder::emit_movsx(X64Register reg, X64Register rm,
...
void X64Builder::emit_operation(u8 opcode, X64Register reg, X64Register rm,
                                InstructionControl control) {
...
void X64Builder::emit_movzx(X64Register reg, X64Register rm,
                            InstructionControl control) {
  Assert(!IS_REG_XMM(reg) && !IS_REG_XMM(rm));
  Assert(!IS_CONTROL_FLOAT(control));
  Assert(
...
void X64Builder::emit_modrm_sib_slash(u8 mod, u8 reg, u8 scale, u8 index,
                                      X64Register _base) {
  u8 base = _base - 1;
  //  register to register (mod = 0b11) doesn't work with SIB byte
  Assert(("Use addModRM instead", mod != 0b11));
...
void X64Builder::emit_prefix(u8 inherited_prefix, X64Register reg,
                             X64Register rm) {
  inherited_prefix = construct_prefix(inherited_prefix, reg, rm);
  if (inherited_prefix)
    emit1(inherited_prefix);
}
u8 X64Builder::construct_prefix(u8 inherited_prefix, X64Register reg,
                                X64Register rm) {
  if (reg != X64_REG_INVALID) {
...
void X64Builder::emit_mov_reg_mem(X64Register reg, X64Register rm,
                                  InstructionControl control, int disp) {
  ...
  if (IS_REG_XMM(reg)) {
    ...
    if (size == CONTROL_64B ||
        size == CONTROL_8B) { // REX is needed with MOV_REG8_RM8, we want to
...
void X64Builder::emit_mov_mem_reg(X64Register rm, X64Register reg,
                                  InstructionControl control, int disp) {
  int size = GET_CONTROL_SIZE(control);
  // if (IS_CONTROL_FLOAT(control)) {
  if (IS_REG_XMM(reg)) {
