...
bool String::copy(String* str){
    if(!str->memory.resize(memory.used))
        return false;
    memcpy(str->memory.data,memory.data,memory.used);
...
std::string& operator+=(std::string& str, String& str2){
    ...
    memcpy(str.data()+len,str2.memory.data,str2.memory.used);
    return str;
}
...
String::operator std::string(){
    ...
    memcpy(out.data(),memory.data,memory.used);
    return out;
}
void PrintRawString(String& str){
    for(int i=0;i<str.memory.used;i++){
